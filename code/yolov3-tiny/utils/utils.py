...
def ap_per_class(tp, conf, pred_cls, target_cls,verbose,iou_thres):
    ...
    # Arguments
        ...
        model : train model ,test None
        iou_thresh : iou threshhold
    # Returns
    ...
    for ci, c in enumerate(unique_classes):
        ...
        if n_p == 0 or n_gt == 0:
            ...
            if verbose is not False:
                np.save('./output/recall_{}_{}.npy'.format(ci,iou_thres),recall)
                np.save('./output/precision_{}_{}.npy'.format(ci,iou_thres), precision)
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, multi_label=True, classes=None, agnostic=False,soft =False,sigma=1):
    """
...
def non_max_suppression_2(prediction, conf_thres=0.1, nms_thres= 0.5):
    """
